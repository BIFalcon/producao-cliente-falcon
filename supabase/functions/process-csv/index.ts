...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "process") {
      ...
      return new Response(
        JSON.stringify({ success: true, action: "process", status: "processing", tenant_id, batch_id }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    ...
    if (chunk_index === 0 && mode === "replace") {
      ...
      const { error: delRawErr } = await supabase.from("raw_reservations").delete().eq("tenant_id", tenant_id);
      if (delRawErr) {
        return errorResponse("delete-raw", delRawErr.message, 500, { tenant_id });
      }
      const { error: delProcErr } = await supabase.from("processed_reservations").delete().eq("tenant_id", tenant_id);
      if (delProcErr) {
    ...
    for (let i = 0; i < processedRows.length; i += batchSize) {
      ...
      if (insertError) {
        return errorResponse("insert-raw", insertError.message, 500, {
          ...
          chunk_index,
          sub_batch_start: i,
          sub_batch_size: subBatch.length,
        });
      }
