...
BEGIN
  ...
  LOOP
    DROP TABLE IF EXISTS tmp_base_rows, tmp_latest_batch, tmp_valid_rows,
      ...
      tmp_texts, tmp_text_priority, tmp_row_matches, tmp_mapped_channels;
    ...
    CREATE INDEX idx_tmp_valid_ct ON tmp_valid_rows (combined_text);
    CREATE INDEX idx_tmp_valid_mt ON tmp_valid_rows (mapping_text);
    ANALYZE tmp_valid_rows;
    ...
    ANALYZE tmp_reservation_totals;

    CREATE TEMP TABLE tmp_res_flags ON COMMIT DROP AS
    SELECT v.confirmation_number, v.property_name,
      BOOL_OR(v.rate_code_desc_normalized ~ 'grupo|group') AS has_group,
      BOOL_OR(
        v.company_name IS NOT NULL OR v.travel_agent_name IS NOT NULL OR v.source_name IS NOT NULL
      ) AS has_identity,
      ...
      BOOL_OR(
        (v.combined_text LIKE '%azul linhas aereas%' OR v.combined_text LIKE '%azul linhas global master%'
         OR v.combined_text LIKE '%layover%' OR v.combined_text LIKE '%lay over%')
    ...
    ANALYZE tmp_channel_mapping;

    CREATE TEMP TABLE tmp_texts ON COMMIT DROP AS
    SELECT DISTINCT v.mapping_text AS ct
    FROM tmp_valid_rows v
    WHERE COALESCE(BTRIM(v.mapping_text), '') <> '';

    ANALYZE tmp_texts;

    CREATE TEMP TABLE tmp_text_priority ON COMMIT DROP AS
    SELECT DISTINCT ON (t.ct)
      t.ct, m.segmento_standard, m.priority
    FROM tmp_texts t
    JOIN tmp_channel_mapping m
      ON POSITION(m.canal_normalized IN t.ct) > 0
     AND t.ct ~ m.canal_regex
    WHERE m.canal_normalized <> ''
    ORDER BY t.ct, m.priority;

    CREATE INDEX idx_tmp_text_priority_ct ON tmp_text_priority (ct);
    ANALYZE tmp_text_priority;
    ...
    FROM tmp_valid_rows v
    JOIN tmp_text_priority tp ON tp.ct = v.mapping_text;

    ANALYZE tmp_row_matches;
...
GRANT EXECUTE ON FUNCTION public.process_reservations(uuid, uuid, text) TO authenticated, service_role;
