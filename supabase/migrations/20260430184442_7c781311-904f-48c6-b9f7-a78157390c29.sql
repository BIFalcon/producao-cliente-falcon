...
BEGIN
  ...
  SELECT DISTINCT ON (v.confirmation_number, v.property_name, m.canal_normalized)
    ...
    m.segmento_standard, m.priority
  FROM tmp_valid_rows v
  ...
  CREATE INDEX idx_tmp_matches_cn_pn ON tmp_row_matches (confirmation_number, property_name);
  ...
  JOIN tmp_reservation_totals rt ON rt.confirmation_number = rm.confirmation_number AND rt.property_name = rm.property_name
    ...
    CASE
      ...
      WHEN EXISTS (
        ...
        WHERE v2.confirmation_number = rt.confirmation_number
          ...
          AND v2.room_type_normalized LIKE '%pm%'
      ) THEN 'Outras receitas (PM e PF)'
      WHEN EXISTS (
        SELECT 1 FROM tmp_valid_rows v2
        WHERE v2.confirmation_number = rt.confirmation_number
          AND v2.property_name = rt.property_name
          AND v2.combined_text ~ '(booking|expedia|decolar|hoteis\\.com|hotelbeds|webbeds|trend)'
      ) THEN 'OTA'
      ELSE 'Particular'
