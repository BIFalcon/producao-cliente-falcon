...
BEGIN
  ...
  INNER JOIN tmp_channel_mapping m
    ON POSITION(m.canal_normalized IN v.combined_text) > 0;
  ...
  SELECT
    v.confirmation_number, v.property_name,
    BOOL_OR(
      POSITION('booking' IN v.combined_text) > 0
  ...
  -- LAYOVER: Azul rules + generic "layover" keyword (now includes individual_first_name via combined_text)
  ...
  SELECT
    ...
    OR
    BOOL_OR(POSITION('layover' IN v.combined_text) > 0)
    AS is_layover
  FROM tmp_valid_rows v
  GROUP BY v.confirmation_number, v.property_name;
  ...
  SELECT
    v.confirmation_number, v.property_name,
    BOOL_OR(
      POSITION('e-htl' IN v.combined_text) > 0
      OR POSITION('ehtl' IN v.combined_text) > 0
      OR POSITION('e htl' IN v.combined_text) > 0
      OR POSITION('azul viagens' IN v.combined_text) > 0
    ) AS is_operadora
  FROM tmp_valid_rows v
  GROUP BY v.confirmation_number, v.property_name;
  ...
  SELECT
    ...
    CASE
      ...
      -- 7. PM/PF (only if NO operational room AND not layover)
  ...
  -- OTA safety net: reclassify invalid OTA rows
