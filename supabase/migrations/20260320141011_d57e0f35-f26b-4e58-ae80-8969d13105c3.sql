...
BEGIN
  ...
  -- Channel mapping: EXCLUDE OTA segment entirely
  ...
  WHERE UPPER(BTRIM(cm.segmento)) <> 'OTA';
  ...
  SELECT DISTINCT ON (v.confirmation_number, v.property_name, m.canal_normalized)
    v.confirmation_number, v.property_name,
    v.company_name, v.travel_agent_name, v.source_name,
  ...
  -- Hardcoded OTA: ONLY booking, expedia, decolar
  CREATE TEMP TABLE tmp_ota_flags ON COMMIT DROP AS
  SELECT
    v.confirmation_number, v.property_name,
    BOOL_OR(
      POSITION('booking' IN LOWER(extensions.unaccent(COALESCE(v.company_name, '') || ' ' || COALESCE(v.travel_agent_name, '')))) > 0
      OR POSITION('expedia' IN LOWER(extensions.unaccent(COALESCE(v.company_name, '') || ' ' || COALESCE(v.travel_agent_name, '')))) > 0
      OR POSITION('decolar' IN LOWER(extensions.unaccent(COALESCE(v.company_name, '') || ' ' || COALESCE(v.travel_agent_name, '')))) > 0
    ) AS is_ota
  FROM tmp_valid_rows v
  GROUP BY v.confirmation_number, v.property_name;
  ...
  -- ===== POST-INSERT SAFETY NET: Reclassify any OTA that does NOT match booking/expedia/decolar =====
  UPDATE public.processed_reservations pr
  SET sales_channel = CASE
    WHEN EXISTS (
      SELECT 1 FROM tmp_operadora_flags op
      WHERE op.confirmation_number = pr.confirmation_number AND op.property_name = pr.property_name AND op.is_operadora
    ) THEN 'Operadoras'
    WHEN EXISTS (
      SELECT 1 FROM tmp_layover_flags ly
      WHERE ly.confirmation_number = pr.confirmation_number AND ly.property_name = pr.property_name AND ly.is_layover
    ) THEN 'Layover'
    WHEN EXISTS (
      SELECT 1 FROM tmp_best_mapping bm
      WHERE bm.confirmation_number = pr.confirmation_number AND bm.property_name = pr.property_name
    ) THEN (
      SELECT bm2.segmento_standard FROM tmp_best_mapping bm2
      WHERE bm2.confirmation_number = pr.confirmation_number AND bm2.property_name = pr.property_name
    )
    WHEN EXISTS (
      SELECT 1 FROM tmp_group_flags g
      WHERE g.confirmation_number = pr.confirmation_number AND g.property_name = pr.property_name AND g.has_group
    ) THEN 'Grupos'
  ...
  WHERE pr.sales_channel = 'OTA'
    AND NOT (
      POSITION('booking' IN LOWER(extensions.unaccent(COALESCE(pr.company_name, '') || ' ' || COALESCE(pr.travel_agent_name, '')))) > 0
      OR POSITION('expedia' IN LOWER(extensions.unaccent(COALESCE(pr.company_name, '') || ' ' || COALESCE(pr.travel_agent_name, '')))) > 0
      OR POSITION('decolar' IN LOWER(extensions.unaccent(COALESCE(pr.company_name, '') || ' ' || COALESCE(pr.travel_agent_name, '')))) > 0
    );
