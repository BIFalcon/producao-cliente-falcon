...
BEGIN
  ...
  SELECT
    ...
    agg.total_revenue,
    agg.lead_time_days,
    -- STEP 3: If any row in the reservation is non-Particular, use the best non-Particular channel
    -- Otherwise use Particular only if ALL rows are Particular
    CASE
  ...
  FROM (
    SELECT
      ...
      -- STEP 1 & 2: Check if ANY row classifies as non-Particular
      BOOL_OR(
        CASE
      ...
      -- Best non-Particular channel by priority (pick the highest priority found)
      CASE
