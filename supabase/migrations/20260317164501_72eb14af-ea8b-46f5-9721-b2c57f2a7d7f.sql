...
BEGIN
  ...
  SELECT
    ...
    agg.total_revenue,
    agg.lead_time_days,
    -- Classification based on unified source
    CASE
  ...
  FROM (
    SELECT
      ...
      -- Build unified source: concatenate non-empty company_name and travel_agent_name across all rows
      LOWER(
        TRIM(
          COALESCE(
            STRING_AGG(DISTINCT NULLIF(TRIM(r.company_name), ''), ' '),
            ''
          ) || ' ' ||
          COALESCE(
            STRING_AGG(DISTINCT NULLIF(TRIM(r.travel_agent_name), ''), ' '),
            ''
          )
        )
      ) AS unified_source,
      BOOL_OR(r.company_name IS NOT NULL AND TRIM(r.company_name) != '') AS has_company,
      BOOL_OR(r.travel_agent_name IS NOT NULL AND TRIM(r.travel_agent_name) != '') AS has_agent,
      BOOL_OR(r.source_name IS NOT NULL AND TRIM(r.source_name) != '') AS has_source
