...
AS $function$
BEGIN
  -- Stage 1: keep only valid rows and normalize classification inputs
  ...
  -- Stage 2: revenue is always from ALL rows, including PM/PF
  ...
  SELECT
    ...
    CASE
      WHEN MIN(v.reservation_date) IS NOT NULL AND MIN(v.arrival_date) IS NOT NULL
      THEN (MIN(v.arrival_date) - MIN(v.reservation_date))
      ELSE NULL
    END AS lead_time_days
  FROM tmp_valid_rows v
  GROUP BY v.confirmation_number, v.property_name;

  -- Stage 3: PM/PF do not define channel; only non-PM/PF rows produce signals
  ...
  FROM (
    SELECT
      ...
      (
        ...
        POSITION('decolar' IN LOWER(CONCAT_WS(' ', v.company_name, v.travel_agent_name, v.source_name))) > 0
      ) AS has_ota,
      (
        ...
        POSITION('klas.turismo' IN LOWER(CONCAT_WS(' ', v.company_name, v.travel_agent_name, v.source_name))) > 0
      ) AS has_operadora,
      (
        POSITION('azul linhas aereas' IN LOWER(CONCAT_WS(' ', v.company_name, v.travel_agent_name, v.source_name))) > 0 AND
        POSITION('azul viagens' IN LOWER(CONCAT_WS(' ', v.company_name, v.travel_agent_name, v.source_name))) = 0
      ) AS has_layover,
  ...
  -- Final write: exactly one reservation row and exactly one category per reservation
  ...
  LEFT JOIN tmp_operational_signals s
    ON s.confirmation_number = t.confirmation_number
